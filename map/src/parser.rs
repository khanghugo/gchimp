...
use crate::{Attributes, Brush, BrushPlane, Entity, Map, TextureName};
...
pub fn parse_plane_coordinate(i: &'_ str) -> IResult<'_, DVec3> {
    terminated(
        preceded(
...
pub fn parse_plane_uv(i: &'_ str) -> IResult<'_, DVec4> {
    terminated(
        preceded(
            tuple((space0, tag("["), space0)),
            map(
...
pub fn parse_brush_plane(i: &'_ str) -> IResult<'_, BrushPlane> {
    map(
        tuple((
        ...
        |(p1, p2, p3, texture_name, u, v, rotation, u_scale, v_scale)| BrushPlane {
            ...
            p2,
            p3,
            texture_name: TextureName::new(texture_name),
            u,
            v,
            rotation,
            u_scale,
...
pub fn parse_brush(i: &'_ str) -> IResult<'_, Brush> {
    map(
...
pub fn parse_brushes(i: &'_ str) -> IResult<'_, Vec<Brush>> {
    many1(delimited(
...
// For attributes
pub fn parse_attribute(i: &'_ str) -> IResult<'_, (&str, &'_ str)> {
...
// For map
pub fn parse_entity(i: &'_ str) -> IResult<'_, Entity> {
    map(
...
pub fn parse_entities(i: &'_ str) -> IResult<'_, Vec<Entity>> {
    many1(delimited(
        discard_comment_lines,
...
pub fn parse_map(i: &'_ str) -> IResult<'_, Map> {
    map(
