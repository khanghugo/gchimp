...
type Palette = (
    Vec<quantette::palette::rgb::Rgb<quantette::palette::encoding::Srgb, u8>>,
    Vec<u8>,
);

fn quantize_to_8bit(img: RgbImage) -> eyre::Result<(RgbImage, Palette)> {
    ...
    let palette: Palette = pipeline.indexed_palette_par();
...
fn png_to_bmp(img_path: &Path) -> eyre::Result<()> {
    ...
    let (width, height) = img.dimensions();
    let (img, (palette_color, palette_index)) = quantize_to_8bit(img)?;
    ...
    let palette_color_arr = palette_color.iter().map(|p| [p.red, p.green, p.blue]).collect::<Vec<[u8; 3]>>();
    let img_bmp_8pp = img
        .chunks_exact(3)
        .map(|p| {
            // unwrap is guaranteed because img uses palette colors
            let index_for_color = palette_color_arr.iter()
                .position(|pp| *pp == [p[0], p[1], p[2]])
                .unwrap();
            palette_index[index_for_color] as u8
        })
        .collect::<Vec<u8>>();
