...
use crate::{writer::WriteToWriter, Bodypart, BodypartHeader, Mdl, MeshHeader, Model, ModelHeader};
...
//         writer.append_u8_slice(name);
//         writer.append_i32(*type_);
//         writer.append_f32(*bounding_radius);
//         writer.append_i32(meshes.len() as i32);

//         let mesh_index = writer.get_offset();
//         writer.append_i32(PADDING_MAGIC);

//         let num_verts = writer.get_offset();
//         writer.append_i32(PADDING_MAGIC);
...
//         let num_norms = writer.get_offset();
//         writer.append_i32(PADDING_MAGIC);
...
//         // not sure what to do with group yet
//         writer.append_i32(0);
//         writer.append_i32(0);

//         // write vertex info first because it is easier
//         writer.replace_with_i32(vert_info_index, writer.get_offset() as i32);
//         writer.append_u8_slice(&vertex_info);

//         // now write all meshes
//         writer.replace_with_i32(mesh_index, writer.get_offset() as i32);

//         // mesh headers are contiguous
//         // so we need to write all headers then write the triangles
//         let index_offsets: Vec<(usize, usize)> = meshes
//             .iter()
//             .map(|mesh| {
//                 let Mesh { header, triangles } = mesh;
//                 let MeshHeader {
//                     num_tris: _,
//                     tri_index: _,
//                     skin_ref,
...
//                 writer.append_i32(triangles.len() as i32); // num tris

//                 let tri_index = writer.get_offset();
//                 writer.append_i32(PADDING_MAGIC);

//                 writer.append_i32(*skin_ref);

//                 // trivert count is triangle count and normal count
//                 writer.append_i32(triangles.len() as i32);
...
//         // write all vertices into its own buffer then join it later
//         // all vertices are used within a model, shared between meshes
//         // so, it must be in one buffer for that one model
//         let mut mesh_vert_writer = ByteWriter::new();
//         let mut mesh_norm_writer = ByteWriter::new();

//         meshes.iter().for_each(|mesh| {
//             // offset starting from each mesh
//             let vert_offset = mesh_vert_writer.get_offset();

//             mesh.triangles.iter().for_each(|mesh_triangles| {
//                 let triverts = mesh_triangles.get_triverts();

//                 triverts.iter().for_each(|trivert| {
//                     mesh_vert_writer.append_f32_slice(trivert.vertex.to_array().as_slice());
//                     mesh_norm_writer.append_f32_slice(trivert.normal.to_array().as_slice());
//                 });
//             });
...
//         // writing the trivert headers
//         let mut vertex_count = 0;

//         meshes
//             .iter()
//             .zip(index_offsets)
//             .for_each(|(mesh, (tri_index, _norm_index))| {
//                 // tri_index points to the first trivert, which is just the header
//                 writer.replace_with_i32(tri_index, writer.get_offset() as i32);
...
//                     // just add as it goes because in the vertex block, we write it incrementally
//                     mesh_triangles.get_triverts().iter().for_each(|trivert| {
//                         writer.append_i16(vertex_count);
//                         writer.append_i16(vertex_count);
//                         writer.append_i16(trivert.header.s);
//                         writer.append_i16(trivert.header.t);
...
//         // now append the vert and norm block
//         writer.replace_with_i32(num_verts, vertex_count as i32);
//         writer.replace_with_i32(num_norms, vertex_count as i32);

//         writer.replace_with_i32(vert_index, writer.get_offset() as i32);
//         writer.append_u8_slice(&mesh_vert_writer.data);

//         writer.replace_with_i32(norm_index, writer.get_offset() as i32);
//         writer.append_u8_slice(&mesh_norm_writer.data);
